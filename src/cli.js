const { stdin, argv, exit } = require('node:process')
...
program
  ...
  .option(
    '-b, --buffer',
    'output the JSON representation of buffer of the chart file (will not output the chart file)'
  )

/**
 * Get options from command line
 *
 * @returns {Promise<{ input?: string, output?: string, format: string, width: string, height: string, buffer: boolean }>}
 */
const __getOptions = () =>
 ...
 * Get options from command line
 *
 * @returns {Promise<{ option: any; output: string; format: string; width: number; height: number, buffer: boolean }>}
 */
async function getOptions() {
  const options = await __getOptions()
  const { input, output, buffer } = options
  ...
  try {
    ...
    return { option, output, format, width, height, buffer }
  } catch (_) {
    try {
...
/**
 *
 * @param { { input?: string, output?: string, format: string, width: string, height: string, buffer: boolean } } opts
 * @param {string[]} args
 *
 * @returns { { input?: string, output?: string, format: string, width: string, height: string, buffer: boolean } }
 */
